...
void GeometryHelper::CreateQuad(shared_ptr<Geometry<VertexColorData>> geometry, Color color)
{
	...
	vtx[0].position = Vec3(0.f, 0.5f, 0.f);
	vtx[0].color = color;
	vtx[1].position = Vec3(0.f, 0.5f, 1.f);
	vtx[1].color = color;
	vtx[2].position = Vec3(1.f, 0.5f, 0.f);
	vtx[2].color = color;
	vtx[3].position = Vec3(1.f, 0.5f, 1.f);
	vtx[3].color = color;
...
void GeometryHelper::CreateCube(Geometry<VertexColorData>& geometry, Color color)
{
	float w2 = 0.5f;
	float h2 = 0.5f;
	float d2 = 0.5f;
	...
	// æ’∏È
	vtx[0] = VertexColorData{ Vec3(-w2, -h2, -d2), color };
	vtx[1] = VertexColorData{ Vec3(-w2, +h2, -d2), color };
	vtx[2] = VertexColorData{ Vec3(+w2, +h2, -d2), color };
	vtx[3] = VertexColorData{ Vec3(+w2, -h2, -d2), color };
	// µﬁ∏È
	vtx[4] = VertexColorData{ Vec3(-w2, -h2, +d2), color };
	vtx[5] = VertexColorData{ Vec3(+w2, -h2, +d2), color };
	vtx[6] = VertexColorData{ Vec3(+w2, +h2, +d2), color };
	vtx[7] = VertexColorData{ Vec3(-w2, +h2, +d2), color };
	// ¿≠∏È
	vtx[8] = VertexColorData{ Vec3(-w2, +h2, -d2), color };
	vtx[9] = VertexColorData{ Vec3(-w2, +h2, +d2), color };
	vtx[10] = VertexColorData{ Vec3(+w2, +h2, +d2), color };
	vtx[11] = VertexColorData{ Vec3(+w2, +h2, -d2), color };
	// æ∆∑ß∏È
	vtx[12] = VertexColorData{ Vec3(-w2, -h2, -d2), color };
	vtx[13] = VertexColorData{ Vec3(+w2, -h2, -d2), color };
	vtx[14] = VertexColorData{ Vec3(+w2, -h2, +d2), color };
	vtx[15] = VertexColorData{ Vec3(-w2, -h2, +d2), color };
	// øﬁ¬ ∏È
	vtx[16] = VertexColorData{ Vec3(-w2, -h2, +d2), color };
	vtx[17] = VertexColorData{ Vec3(-w2, +h2, +d2), color };
	vtx[18] = VertexColorData{ Vec3(-w2, +h2, -d2), color };
	vtx[19] = VertexColorData{ Vec3(-w2, -h2, -d2), color };
	// ø¿∏•¬ ∏È
	vtx[20] = VertexColorData{ Vec3(+w2, -h2, -d2), color };
	vtx[21] = VertexColorData{ Vec3(+w2, +h2, -d2), color };
	vtx[22] = VertexColorData{ Vec3(+w2, +h2, +d2), color };
	vtx[23] = VertexColorData{ Vec3(+w2, -h2, +d2), color };
...
void GeometryHelper::CreateGrid(shared_ptr<Geometry<VertexTextureData>> geometry, uint32 sizeX, uint32 sizeZ, uint32 divideBy)
{
	vector<VertexTextureData> vtx;

	for (uint32 z = 0; z < sizeZ + 1; z++)
	{
		for (uint32 x = 0; x < sizeX + 1; x++)
		{
			VertexTextureData v;
			v.position = Vec3(static_cast<float>(x), 0, static_cast<float>(z));
			v.uv = Vec2(static_cast<float>(x), static_cast<float>(z));
	...
	vector<uint32> idx;
	vector<vector<uint32>> sub_idx(divideBy * divideBy);

	uint32 subSizeX = sizeX / divideBy;
	uint32 subSizeZ = sizeZ / divideBy;

	for (uint32 z = 0; z < sizeZ; z++)
	{
		for (uint32 x = 0; x < sizeX; x++)
		{
			uint32 subIndex = (z / subSizeZ) * divideBy + (x / subSizeX);
			...
			// «ÿ¥Á ¿Œµ¶Ω∫∏¶ «œ¿ß ¿Œµ¶Ω∫ πˆ∆€ø° √ﬂ∞°
			sub_idx[subIndex].insert(sub_idx[subIndex].end(), {
				(sizeX + 1) * (z + 1) + (x),
				(sizeX + 1) * (z)+(x + 1),
				(sizeX + 1) * (z)+(x)
				});
	...
	geometry->SetIndices(idx);
	geometry->SetSubIndices(sub_idx);
}
